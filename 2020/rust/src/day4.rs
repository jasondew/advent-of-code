...
impl Passport {
    fn load(input: &str) -> Vec<Passport> {
        input
            .split("\n\n")
...
fn valid_height(string: &str) -> bool {
    if let Some(rest) = string.strip_suffix("cm") {
        between(&rest.to_owned(), 150, 193)
    } else {
        string
            .strip_suffix("in")
            .map_or(false, |rest| between(&rest.to_owned(), 59, 76))
    }
}

fn valid_hair_color(string: &str) -> bool {
    string
        .strip_prefix("#")
        .map_or(false, |rest| i32::from_str_radix(rest, 16).is_ok())
}

fn valid_eye_color(string: &str) -> bool {
    ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(&string)
}

fn valid_passport_id(string: &str) -> bool {
...
pub fn part1(input: &str) -> usize {
    let passports: Vec<Passport> = Passport::load(input);
    passports
        .iter()
