defmodule ElixirSolutionsTest do
  use ExUnit.Case

  doctest ElixirSolutions.DayOne
  doctest ElixirSolutions.DayTwo
  doctest ElixirSolutions.DayThree
  doctest ElixirSolutions.DayFour
  doctest ElixirSolutions.DayFive
  doctest ElixirSolutions.DaySix
  doctest ElixirSolutions.DaySeven
  doctest ElixirSolutions.DayEight
  doctest ElixirSolutions.DayNine
  doctest ElixirSolutions.DayTen
  doctest ElixirSolutions.DayEleven
  doctest ElixirSolutions.DayTwelve
  doctest ElixirSolutions.DayThirteen
  doctest ElixirSolutions.DayFourteen
  doctest ElixirSolutions.DayFifteen, import: true
  doctest ElixirSolutions.DaySixteen, import: true
  doctest ElixirSolutions.DayNineteen, import: true
  doctest ElixirSolutions.DayTwentyThree, import: true
end
